...
def put_hot_pixels_in_voxel(voxel, hot_pixel_range=20, hot_pixel_fraction=0.00002):
    # 计算热点像素数量
    # voxel hwc
    ...
    # 随机生成热点像素的值
    hot_pixel_values = np.random.randint(-hot_pixel_range, hot_pixel_range + 1, size=num_hot_pixels)
...
def add_noise_to_img(img, noise_std=1.0, noise_fraction=0.1):
    ...
    noise_std = np.random.uniform(1,10) 
...
class VoxelnpzPngSingleDeblurDaDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(VoxelnpzPngSingleDeblurDaDataset, self).__init__()
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        # voxel shape: h,w,c
        # random resize
        resize_prob = random.random() < 0.5
        # random_scale = (random.random()-0.5)*0.2/0.5 + 1.0
        random_scale = random.random() + 0.5

        random_scale_w = (random.random()-0.5)/5.0
        random_scale_h = (random.random()-0.5)/5.0
...
if __name__ == "__main__":
    ...
    dataset = VoxelnpzPngSingleDeblurDaDataset(opt)
