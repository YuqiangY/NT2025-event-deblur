...
from basicsr.utils.tta import tta
...
class Nine_ImageEventRestorationModel(BaseModel):
    """Base Event-based deblur model for single image deblur."""

    def __init__(self, opt):
        super(Nine_ImageEventRestorationModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('edge_opt'):
            self.edge_type = train_opt['edge_opt'].pop('type')
            # print('LOSS: pixel_type:{}'.format(self.pixel_type))
            cri_edge_cls = getattr(loss_module, self.edge_type)
        ...
        if train_opt.get('psnry_opt'):
            self.psnry_type = train_opt['psnry_opt'].pop('type')
            # print('LOSS: pixel_type:{}'.format(self.pixel_type))
            cri_psnry_cls = getattr(loss_module, self.psnry_type)

            self.cri_psnry = cri_psnry_cls(**train_opt['psnry_opt']).to(
                self.device)
        else:
            self.cri_psnry = None
    ...
    def grids_voxel(self):
        ...
        while i < h and not last_i:
            ...
            if i + crop_size >= h:
                i = h - crop_size
                last_i = True
            overlap_top, overlap_bottom = overlap, overlap
            if i - overlap < 0:
                overlap_top = i
                overlap_bottom = overlap * 2 - overlap_top
            if i + crop_size + overlap_bottom >= h:
                overlap_bottom = h - i - crop_size
                overlap_top = overlap * 2 - overlap_bottom
            ...
            while j < w and not last_j:
                if j + crop_size >= w:
                    j = w - crop_size
                    last_j = True
                overlap_left, overlap_right = overlap, overlap
                if j - overlap < 0:
                    overlap_left = j
                    overlap_right = overlap * 2 - overlap_left
                if j + crop_size + overlap_right >= w:
                    overlap_right = w - j - crop_size
                    overlap_left = overlap * 2 - overlap_right
                # from i, j to i+crop_szie, j + crop_size
                # print(' trans 8')
                for trans_idx in range(self.opt['val'].get('trans_num', 1)):
                    parts.append(self.transpose(self.voxel[:, :, i-overlap_top:i + crop_size+overlap_bottom, 
                                                        j-overlap_left:j + crop_size + overlap_right], trans_idx))
    ...
    def grids(self):
        ...
        assert b == 1
        crop_size = self.opt['val'].get('crop_size')
        overlap = self.opt['val'].get('overlap', 0)
        ...
        while i < h and not last_i:
            ...
            while j < w and not last_j:
                ...
                # print(' trans 8')
                for trans_idx in range(self.opt['val'].get('trans_num', 1)):
                    parts.append(self.transpose(self.lq[:, :, i-overlap_top:i + crop_size+overlap_bottom, 
                                                        j-overlap_left:j + crop_size + overlap_right], trans_idx))
                    idxes.append({'i': i, 'j': j, 'trans_idx': trans_idx, 'overlap': (overlap_top, overlap_bottom, overlap_left, overlap_right)})
                    # cnt_idx += 1
                j = j + step_j
            i = i + step_i
        if self.opt['val'].get('random_crop_num', 0) > 0:
    ...
    def grids_inverse(self):
        ...
        for cnt, each_idx in enumerate(self.idxes):
            ...
            output_i = self.output[cnt, :, :, :]
            if self.overlap > 0:
                ph, pw = output_i.shape[-2:]
                overlap_top, overlap_bottom, overlap_left, overlap_right = each_idx['overlap']
                output_i = output_i[:, overlap_top:ph-overlap_bottom, overlap_left:pw-overlap_right]
            preds[0, :, i:i + crop_size, j:j + crop_size] += self.transpose_inverse(output_i.unsqueeze(0), trans_idx).squeeze(0)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_psnry:
            ...
            if self.psnry_type == 'PSNRYLoss':
                for pred in preds:
                    l_psnry += self.cri_psnry(pred, self.gt)
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if 'val' in self.opt['datasets'] and self.opt['datasets']['val'].get('use_mask'):
                    ...
                    # if self.opt['val'].get('tta', None) is not None:
                    #     tta_mode = self.opt['val']['tta']
                    #     print(tta_mode)
                    #     pred = tta(self.lq[i:j, :, :, :], self.voxel[i:j, :, :, :], self.net_g, tta_mode)
                    # else:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if self.opt['val'].get('multi_grids') is not None:
                crop_sizes = self.opt['val']['crop_sizes']
                overlaps = self.opt['val']['overlaps']
                sr_img_list = []
                for crop_size, overlap in zip(crop_sizes, overlaps):
                    self.opt['val']['crop_size'] = crop_size
                    self.opt['val']['overlap'] = overlap
                    ...
                    sr_img_list.append(result)
                sr_img = torch.stack(sr_img_list, dim=0)
                sr_img = torch.mean(sr_img, dim=0)
                sr_img = tensor2img([sr_img], rgb2bgr=rgb2bgr)
            else:
                if self.opt['val'].get('grids') is not None:
                    self.grids()
                    self.grids_voxel()
